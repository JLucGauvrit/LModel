...
<body>
  ...
  <script>
    ...
    function updateCharts(history) {
      ...
      const metricKeys = Object.keys(history[0]).filter(k => k !== 'x');

      for (const key of metricKeys) {
        if (!charts[key]) {
          charts[key] = makeChart(key);
        }
        const pts = history.map(p => ({ x: p.x, y: p[key] }));
        charts[key].data.datasets[0].data = pts;
...
async def _event_stream():
    """
    Générateur asynchrone qui surveille runs/status.json et émet un event SSE
    à chaque modification du fichier (détection par mtime).

    :returns: Chaînes SSE formatées ``data: <json>\\n\\n``.
...
async def events() -> StreamingResponse:
    """
    Endpoint Server-Sent Events : diffuse chaque mise à jour de status.json
    dès qu'elle est écrite par les scripts d'entraînement.
